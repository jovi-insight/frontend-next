...
async function main() {
  ...
  try {
    for (const width of [320, 390, 1280]) {
      ...
      assert.equal(await page.locator('.math-read').innerText(), '2+3×4');
      ...
      await resolver.click(); await resultado.filter({ hasText: /^2×x$/ }).waitFor();
      assert.equal(pedidos.at(-1).expressao, 'x^2'); assert.equal(pedidos.at(-1).variavel, 'x');
      assert.match(await page.getByLabel('Passo a passo da resolução').innerText(), /2×x/);
      await page.getByLabel('Operação matemática').selectOption('auto');
